...
<body>
<h2>Data Kategori Kriteria </h2><hr>
<div class="konten">
<?php
...
<form id="form1" name="form1" method="post" action="../relasisim.php" enctype="multipart/form-data"><div>
  <table class="tab" width="528" border="0" align="center" cellpadding="4" cellspacing="1" bordercolor="#F0F0F0" bgcolor="#CCCC99">
      <tr bgcolor="#FFFFFF">
        <td>Kriteria</td>
        <td><label>
        <select name="TxtKdKriteria" id="TxtKdKriteria">
          <?php 
	$kriteria = "SELECT * FROM kriteria ORDER BY kriteria_id";
	$qryp = mysql_query($kriteria, $koneksi) 
	...
	while ($datap=mysql_fetch_array($qryp)) {
		...
		echo "<option value='$datap[kriteria_id]' $cek>$datap[kriteria_idt]&nbsp;|&nbsp;$datap[kriteria_nama]</option>";
	}
  		?>
        </select>
        </label>
      ...
      <tr bgcolor="#FFFFFF">
       <td>Sub Kriteria</td>
          <select name="TxtSubK" id="TxtSubK">
            <?php 
	$kriteria =$_GET['kriteria'];
	$sqlp = "SELECT * FROM sub_kriteria where id_kriteria = '$kriteria' ";
	$qryg = mysql_query($sqlp, $koneksi) 
		    or die ("SQL Error: ".mysql_error());
	while ($datag=mysql_fetch_array($qryg)) {
		if ($datag['sub_id']==$sub_id) {
			$cek ="selected";
		}
		else {
			$cek ="";
		}
		echo "<option value='$datag[sub_id]' $cek>$datag[sub_id]&nbsp;|&nbsp;$datag[sub_nama]</option>";
	}
 			 ?>
          </select>
         </td>
